...
@pytest.mark.parametrize("compression, filters", [
    (
        numcodecs.Zlib(),
        [numcodecs.Shuffle()],
    ),
...
def test_reduce_chunk_compression(mock_request, compression, filters):
    ...
    expected_data = {
        ...
        shape,
        "selection": [[
            chunk_selection[0].start, chunk_selection[0].stop,
            chunk_selection[0].step
        ]],
        "compression": {
            "id": compression.codec_id
        },
        "filters": [{
            "id": filter.codec_id,
...
def test_reduce_chunk_missing(mock_request, missing):
    ...
    expected_data = {
        ...
        "float32",
        "byte_order":
        "little" if sys.byteorder == "big" else "big",
        "offset":
        offset,
        "size":
        size,
